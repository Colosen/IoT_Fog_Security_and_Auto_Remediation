...
def encrypt(plaintext):
    ...
    chiphertext=base64.b64encode(public_key.encrypt(plaintext,padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),algorithm=hashes.SHA256(),label=None)))
    return(chiphertext)
def decrypt(chiphertext):
    chiphertext=private_key.decrypt(base64.b64decode(chiphertext),padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()),algorithm=hashes.SHA256(),label=None))
    return(chiphertext.decode())
